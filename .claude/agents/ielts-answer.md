...
name: ielts-answer
description: >-
  IELTS daily-pack sub-agent. Generate detailed answer keys, Vietnamese
  explanations, model writing answers, and the Review Bridge from the assembled
  reading/grammar/writing/vocabulary sections. Returns an answers JSON section.
  Use when the orchestrator needs the Answer section.
tools: ["Read"]
---
...
3. **Deep Answer Key Contract**: Follow `.claude/skills/ielts-daily-reading-writing/references/deep-answer-key-rules.md`. Do not finalize an answer key that merely repeats evidence, states a formula without meaning/use, or fails to explain why distractors are wrong.
