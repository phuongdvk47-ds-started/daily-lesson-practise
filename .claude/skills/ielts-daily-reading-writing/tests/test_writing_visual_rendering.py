...
class TestWritingVisualRendering(unittest.TestCase):
    
    def setUp(self):
        self.tmp_json_path = Path("tmp_test_writing_visual_source.json")
        self.base_data = {
            "execution": {
                "mode": "auto",
    ...
    def test_prompt_must_not_contain_svg_when_visual_data_exists(self):
        data = dict(self.base_data)
        data["writing"]["tasks"] = [
            {
                "id": 1,
                "task_type": "Data Description",
                "prompt": "Compare <svg>...</svg>",
    ...
    def test_task_type_not_duplicated_in_prompt(self):
        data = dict(self.base_data)
        data["writing"]["tasks"] = [
            {
                "id": 1,
                "task_type": "Word Ordering",
                "prompt": "Word Ordering: Sắp xếp từ thành câu.",
                "target_length": "3 sentences",
    ...
    def test_chart_requires_title_axis_labels_and_unit(self):
        data = dict(self.base_data)
        data["writing"]["tasks"] = [
            {
                "id": 1,
                "task_type": "Data Description",
                "prompt": "Compare the chart below.",
                "target_length": "3 sentences",
                "focus_skill": "Comparison",
                "useful_language": ["more"],
                "success_criteria": ["Compare"],
                "visual_data": {
                    "type": "svg",
                    "content": "<svg><text>OnlyOneLabel</text></svg>"
                },
                "topic_alignment": True
            }
        ]
