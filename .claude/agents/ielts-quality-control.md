---
name: ielts-quality-control
description: >-
  IELTS daily-pack sub-agent. Cross-validate the fully assembled lesson payload
  (source, reading evidence, deep-reading ratios, vocabulary, grammar logic,
  writing alignment, time/workload, answer quality) and return a QC report with
  challenges. Use when the orchestrator needs a quality-control pass or a
  challenge re-check.
tools: ["Read"]
---
...
3. **Reading Deep Comprehension & Anti-Scanning QC**:
   - **CRITICAL REQUIREMENT**: You MUST evaluate the Reading section against the detailed level-by-level rubric in `.claude/skills/ielts-daily-reading-writing/references/deep-reading-qc.md`, the generation table in `.claude/skills/ielts-daily-reading-writing/references/deep-reading-generation-rules.md`, the blueprint contract in `.claude/skills/ielts-daily-reading-writing/references/deep-question-blueprint.md`, and the fail gates in `.claude/skills/ielts-daily-reading-writing/references/regeneration-quality-gates.md`.
   ...
   - For `+` levels, enforce the transition rules described in `.claude/skills/ielts-daily-reading-writing/references/deep-reading-qc.md`.
   - Enforce the maximum/minimum ratio table in `.claude/skills/ielts-daily-reading-writing/references/deep-reading-generation-rules.md`; raise `keyword_scanning` or `level_mismatch` if the section is more literal than allowed.
...
5.1. **Deep Grammar & Anti-Repetition QC**:
   - **CRITICAL REQUIREMENT**: You MUST evaluate the Grammar section against the detailed rubric in `.claude/skills/ielts-daily-reading-writing/references/deep-grammar-rules.md`, the generation table in `.claude/skills/ielts-daily-reading-writing/references/deep-grammar-generation-rules.md`, the blueprint contract in `.claude/skills/ielts-daily-reading-writing/references/deep-question-blueprint.md`, and the fail gates in `.claude/skills/ielts-daily-reading-writing/references/regeneration-quality-gates.md`.
   ...
   - Verify the level-specific distribution from `.claude/skills/ielts-daily-reading-writing/references/deep-grammar-generation-rules.md`. Raise `cognitive_level_imbalance` if the set is mostly mechanical form or misses meaning/context/discourse/writing-transfer requirements.
...
8. **Answers Check**:
   ...
   - Apply `.claude/skills/ielts-daily-reading-writing/references/deep-answer-key-rules.md`: fail answers that only copy evidence, omit distractor-by-distractor reasoning, state grammar formulas without meaning/use, or omit trap/depth checks.
...
10. **JSON Schema Check**:
    - Verify that the payload structure aligns 100% with the schema in `.claude/skills/ielts-daily-reading-writing/references/output-schema.md`.
...
```json
{
  ...
  "deep_reading_qc_report": "If asked to provide a detailed human review or 'Daily IELTS Checker' report, format it strictly according to the 'Output Format: PDF - Daily IELTS Checker' section in .claude/skills/ielts-daily-reading-writing/references/deep-reading-qc.md. Include Executive Summary, Score Breakdown, 3.1 Reading Logic (with Deep Reading distribution and Level expectations), Issues, and Final Recommendation.",
  "challenges": [
    {
      "id": "CHG-001",
...
> Full patterns with examples: `.claude/agents/ielts-grammar.md §Grammar Logic and Ambiguity Prevention Rules`
...
## Regression Cases
> Full patterns with examples: `.claude/agents/ielts-grammar.md §Correct-the-Error Logic Rules`
