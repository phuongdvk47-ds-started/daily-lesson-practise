...
def compile_html_to_pdf(html_path: Path, pdf_path: Path, lesson_id: str = "", doc_type: str = "") -> None:
    try:
        ...
        # Fail loudly: a locked/stale PDF must NOT be silently kept while the
        # export reports success. The caller (main) will abort the whole run.
        print(f"\nERROR: Could not compile PDF {pdf_path.name} because of error: {e}", file=sys.stderr)
        print("The PDF was NOT regenerated. Close any application holding a lock on this file (e.g. a PDF reader) and run the export again.\n", file=sys.stderr)
        raise
    # Defense in depth: confirm the PDF was actually written. Without this, a
    # silent no-op compile would leave a stale file in place and the post-render
    # QC could pass against the wrong (old) PDF.
    if not pdf_path.exists() or pdf_path.stat().st_size == 0:
        raise RuntimeError(
            f"PDF {pdf_path.name} was not written or is empty after compilation; "
            "the previous file (if any) is now stale. Re-run the export."
        )
...
def convert_json_to_markdown_fields(data: dict) -> dict:
    ...
    if gqs:
        ...
        # Renumber grammar questions sequentially by type-group so displayed
        # numbers stay contiguous (1..N) and group range labels are correct.
        seq = 0
        for q_type, q_list in by_type.items():
            for q in q_list:
                seq += 1
                grammar_old_to_new[q["id"]] = seq

        for q_type, q_list in by_type.items():
            pm.append(f"### {q_type}")
            pm.append("")
            for q in q_list:
                new_id = grammar_old_to_new[q["id"]]
                stretch_mark = " (*)" if q.get("stretch") else ""
                options_str = ""
                if q.get("options"):
                    letters = ["A", "B", "C", "D", "E", "F"]
                    options_str = "\n" + "\n".join([f"    {letters[opt_idx]}. {opt}" for opt_idx, opt in enumerate(q["options"])])
                pm.append(f"{new_id}. {q['question']}{stretch_mark}{options_str}")
            pm.append("")
    ...
    if g_ans:
        ...
        am.append("")
        # Sort so answers follow the same sequential type-group order as the practice sheet.
        g_ans_sorted = sorted(
            g_ans,
            key=lambda ga: grammar_old_to_new.get(ga.get("question_id", 0), ga.get("question_id", 0)),
        )
        for idx, ga in enumerate(g_ans_sorted):
